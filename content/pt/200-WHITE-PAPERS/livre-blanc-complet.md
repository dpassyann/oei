...
```mermaid
flowchart TB
    subgraph HAUT["O que é enquadrado: os objetos técnicos"]
        direction LR
        A1["Protocolos<br/>RFC · IETF · W3C"]
        A2["Segurança<br/>ISO/IEC 27001 · OWASP"]
        A3["Segurança crítica<br/>DO-178C · IEC 62304 · ISO 26262"]
    end
    subgraph BAS["O que não é enquadrado: as pessoas"]
        direction LR
        B1["Nenhum título<br/>exigido"]
        B2["Nenhuma deontologia<br/>oponível"]
        B3["Nenhuma formação<br/>contínua exigida"]
    end
    HAUT -. "o mesmo profissional<br/>concebe o sistema crítico" .-> BAS
```
...
```mermaid
flowchart LR
    F["Formação<br/>inicial"] --> C["Certificação<br/>independente"]
    C --> D["Deontologia<br/>comum"]
    D --> FC["Formação<br/>contínua"]
    FC --> T(("Confiança"))
    T -. "mantém e<br/>renova" .-> FC
```
...
```mermaid
flowchart LR
    P["Praticante<br/>fundamentos,<br/>sob supervisão"] --> I["Engenheiro<br/>conceção autónoma<br/>de componentes"]
    I --> A["Arquiteto<br/>coerência de<br/>sistemas inteiros"]
    A --> E["Especialista<br/>referência em<br/>domínios sensíveis"]
    E --> ES["Especialista sénior<br/>responsabilidade crítica<br/>e transmissão"]
    ES --> FE["Fellow<br/>contributo duradouro<br/>e eminente"]
```
